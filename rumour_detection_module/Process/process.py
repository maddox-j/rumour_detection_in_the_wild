...
from rumour_detection_module.Process.dataset import GraphDataset,BiGraphDataset,UdGraphDataset
cwd=os.getcwd()
...
def loadTree(dataname):
    if 'Twitter' or "Pheme" in dataname:
        treePath = os.path.join(Path(os.path.abspath(__file__)).parent.parent,'data/'+dataname+'/data.TD_RvNN.vol_5000.txt')
...
def loadBiData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate,BUdroprate):
    data_path = os.path.join(Path(os.path.abspath(__file__)).parent.parent,'data', dataname + 'graph')
    print("loading train set", )

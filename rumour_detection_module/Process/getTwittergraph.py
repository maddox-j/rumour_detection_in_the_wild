...
def main(obj):
    treePath = os.path.join(Path(os.path.abspath(__file__)).parent.parent, 'data/' + obj + '/data.TD_RvNN.vol_5000.txt')
    ...
    labelPath = os.path.join(Path(os.path.abspath(__file__)).parent.parent, "data/" + obj + "/" + obj + "_label_All.txt")

    labelset_nonR, labelset_f, labelset_t, labelset_u = ['news', 'non-rumor', 'non-rumours'], ['false'], ['true'], ['unverified','rumours']
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez( os.path.join(Path(os.path.abspath(__file__)).parent.parent, 'data/'+obj+'graph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
...
def create_inference_data_representation(tweet_id):
    obj = "inference_data"
    treePath = os.path.join(Path(os.path.abspath(__file__)).parent.parent, 'data/' + obj + '/' + tweet_id + '.txt')
    treeDic = {}
    for line in open(treePath):
        line = line.rstrip()
        eid, indexP, indexC = line.split('\t')[0], line.split('\t')[1], int(line.split('\t')[2])
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            rootfeat, tree, x_x, rootindex, y = np.array(rootfeat), np.array(tree), np.array(x_x), np.array(
                rootindex), np.array(y)
            np.savez( os.path.join(Path(os.path.abspath(__file__)).parent.parent, 'data/'+obj+'_graph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
    print("loading dataset", )
    Parallel(n_jobs=30, backend='threading')(delayed(loadEid)(treeDic[eid] if eid in treeDic else None,eid,labelDic[eid]) for eid in tqdm(event))
    print("loading was done here")
    return
